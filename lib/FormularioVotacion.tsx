...
export default function FormularioVotacion({
  ...
  // Estados para cuando falla el token y piden pegar el link de nuevo
  ...
  useEffect(() => {
    // Si tenemos token, usamos la URL del token. Si no, la del ID.
    const url = token ? `/api/votar/${token}` : `/api/votar?id=${votanteId}`;
  ...
  async function toggleAsistencia() {
    ...
    // Elegimos la URL y los datos dependiendo de cómo entramos
    const patchUrl = token ? `/api/votar/${token}` : `/api/votar`;
    const patchBody = token
      ? { confirmado: nuevoValor }
      : { votanteId, confirmado: nuevoValor };
  ...
  return (
    <div className="contenedor">
      ...
      <p className="subtitulo">
        Puntúa a{" "}
        <strong style={{ color: "var(--acento)" }}>{actual.nombre}</strong> del
        1 al 10 en cada atributo. Tus votos son anónimos para los demás.
      </p>
